...
/**
 * Performs the occlusion culling or k-DOP culling using the dynamic bounding volume tree from the Bullet broad phase.
 * <p>
 ...
 * <p>
 * k-DOP culling determines which objects are inside a camera frustum. The process is accelerated by the dynamic bounding volume tree.
 *
 * @author jsjolund
 */
public abstract class OcclusionCuller implements Disposable {
    ...
    protected class Collider extends ICollide {
        ...
        public boolean AllLeaves(btDbvtNode node) {
            if (node.isleaf()) {
                ...
                if ((child = btDbvtNode.internalTemp(nodePointer, false).getChild(0)).getCPointer() != 0) {
                    AllLeaves(child);
                }
                if ((child = btDbvtNode.internalTemp(nodePointer, false).getChild(1)).getCPointer() != 0) {
                    AllLeaves(child);
                }
            }
            return true;
        ...
        /**
         * Callback for {@link btDbvt#collideOCL}. This method must check if the input node from the broad phase bounding volume
         ...
         *
         * @param node A node from the broad phase bounding volume tree.
         ...
         * @param depth The depth of the node along the sorting axis. Objects closer to the camera will have a value closer to zero.
         */
        @Override
     ...
     * {@link #onObjectVisible(btCollisionObject)}. The process of finding objects in frustum is accelerated by the broad phase
     * dynamic bounding volume tree ({@link btDbvt}).
     *
     * @param broadphase The dynamics world broad phase.
