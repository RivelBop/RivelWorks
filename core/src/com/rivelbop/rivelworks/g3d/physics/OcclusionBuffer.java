...
public class OcclusionBuffer implements Disposable {
            ...
            switch (this) {
                case DRAW:
                    if (newDepth > oldDepth) {
                        depthBuffer.put(bufferIndex, newDepth);
                    }
                    return false;
                case QUERY:
    ...
    static class Quaternion extends com.badlogic.gdx.math.Quaternion {
        ...
        /**
         * Multiply the x,y,z,w components of the passed in quaternion with the scalar and add them to the components of this quaternion.
         */
    ...
    private int clipQuad(Quaternion[] verticesIn, Quaternion[] verticesOut) {
        ...
        if (numVertsBehind == numVerts) {
            ...
            for (int i = numVerts - 1, j = 0; j < numVerts; i = j++) {
                ...
                if ((t > 0) && (t < 1)) {
                    verticesOut[newNumVerts++].set(a).mulAdd(b.sub(a), t);
                }
                if (s[j] > 0) {
                    verticesOut[newNumVerts++].set(verticesIn[j]);
                }
            }
            return newNumVerts;
        } else {
    ...
    private boolean drawTriangle(Vector3 a, Vector3 b, Vector3 c, Policy policy) {
        ...
        if (((tmpV1.set(b).sub(a)).crs(tmpV2.set(c).sub(a))).z <= 0) {
            return false;
        }
        ...
        for (int iy = yMin; iy < yMax; iy++) {
            for (int ix = xMin; ix < xMax; ix++) {
                ...
                if (cursor.x >= 0 && cursor.y >= 0 && cursor.z >= 0 && policy.process(buffer, bufferIndex, drawDepth)) {
                    return true;
                }
                cursor.add(triDX);
                drawDepth += dzx;
